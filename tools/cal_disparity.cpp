...
//        DrawDisparity(disparityLeft); //for debug
        cv::imwrite(disparityPath,disparityLeft);
    }
    return 0;
}
...
void DrawDisparity(const cv::Mat& imDis)
{
    ...
    for (int i = 0; i < imDis.rows; ++i) {
        for (int j = 0; j < imDis.cols; ++j) {
            if (maxVal < imDis.at<float>(i, j)) {
                maxVal = imDis.at<float>(i, j);
            }
        }
    }
    ...
    for (int i = 0; i < imDis.rows; ++i) {
        for (int j = 0; j < imDis.cols; ++j) {
            float valf = 255.0 * imDis.at<float>(i, j) / (float)maxVal;
